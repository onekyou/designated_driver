...
class MyFirebaseMessagingService : FirebaseMessagingService() {
    ...
    override fun onMessageReceived(remoteMessage: RemoteMessage) {
        ...
        // DRIVER_STATUS_UPDATE는 callId가 없어도 처리 가능
        val callId = remoteMessage.data["callId"]
            ?: remoteMessage.data["sharedCallId"]
            ?: if (messageType == "DRIVER_STATUS_UPDATE") "no_call_id" else return
        ...
        when (messageType) {
            ...
            "CALL_STATUS_UPDATE" -> {
                ...
                handleCallStatusUpdate(remoteMessage, callId)
    ...
    private fun handleCallStatusUpdate(remoteMessage: RemoteMessage, callId: String) {
        Log.d(TAG, "📊 [CALL_STATUS_UPDATE] callId=$callId")

        val status = remoteMessage.data["status"]
        val departure = remoteMessage.data["departure"]
        val destination = remoteMessage.data["destination"]

        Log.d(TAG, "📊 [CALL_STATUS_UPDATE] status=$status, departure=$departure, destination=$destination")

        // 로컬 DB 업데이트
        Thread {
            try {
                val db = com.designated.callmanager.data.local.AppDatabase.getDatabase(applicationContext)

                // 상태 업데이트
                if (status != null) {
                    kotlinx.coroutines.runBlocking {
                        db.callDao().updateCallStatus(
                            callId = callId,
                            status = status,
                            assignedDriverId = null
                        )
                    }
                    Log.d(TAG, "✅ [CALL_STATUS_UPDATE] status 업데이트 완료: $status")
                }

                // 출발지/목적지 업데이트
                if (departure != null || destination != null) {
                    kotlinx.coroutines.runBlocking {
                        db.callDao().updateDestinationInfo(
                            callId = callId,
                            departure = departure,
                            destination = destination
                        )
                    }
                    Log.d(TAG, "✅ [CALL_STATUS_UPDATE] departure/destination 업데이트 완료")
                }

            } catch (e: Exception) {
                Log.e(TAG, "❌ [CALL_STATUS_UPDATE] DB 업데이트 실패", e)
            }
        }.start()
    }
    ...
    private fun handleDriverStatusUpdate(remoteMessage: RemoteMessage, callId: String) {
        ...
        val regionId = remoteMessage.data["regionId"]
        val officeId = remoteMessage.data["officeId"]
        ...
        Thread {
            try {
                val db = com.designated.callmanager.data.local.AppDatabase.getDatabase(applicationContext)
                kotlinx.coroutines.runBlocking {
                    db.driverDao().updateDriverStatus(
                        driverId = driverId,
                        status = newStatus
                    )
                }
                Log.d(TAG, "✅ [DRIVER_STATUS_UPDATE] DB 업데이트 완료: $driverId -> $newStatus")
            } catch (e: Exception) {
                Log.e(TAG, "❌ [DRIVER_STATUS_UPDATE] DB 업데이트 실패", e)
            }
        }.start()
        ...
        if (!isInForeground) {
            ...
            Log.d(TAG, "🔔 [DEBUG] 포그라운드 상태 - DRIVER_STATUS_UPDATE 알림 생략")
        }
    }
