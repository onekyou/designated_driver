...
function buildPrompt(recordedAt: string): string {
  return [
...
async function runGemini(
  ...
  const ai = new GoogleGenAI({ vertexai: true, project: PROJECT, location: VERTEX_LOCATION });
  ...
  try {
    ...
    throw new HttpsError("internal", "Gemini 응답 JSON 파싱 실패");
  }
  return { reservation, latencyMs };
}

/** gs://bucket/path → { bucket, path } */
function parseGcsUri(uri: string): { bucket: string; path: string } | null {
  const m = /^gs:\/\/([^/]+)\/(.+)$/.exec(uri);
  if (!m) return null;
  return { bucket: m[1], path: m[2] };
}

export const parseReservation = onCall(
  ...
  async (request) => {
    ...
    if (!gcsUri && !transcript) {
      throw new HttpsError("invalid-argument", "gcsUri 또는 transcript 중 하나는 필요합니다.");
    }
