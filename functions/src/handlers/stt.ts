 ...
 */
export async function transcribeAudio(gcsUri: string): Promise<string> {
  if (!SERVICE_URL) {
  ...
  const data = (await resp.json()) as { transcript?: string; model?: string };
  logger.info("[stt] 전사 완료", {
    latencyMs: Date.now() - start,
    model: data.model,
    len: data.transcript?.length ?? 0,
  });
