...
export async function processSharedCallPoints(
  ...
  try {
    ...
    const r = cfgSnap.data()?.sharedCallRatio;
...
export async function refundCustomerPointsOnCancel(
  provinceId: string,
  cityId: string,
  officeId: string,
  callId: string,
 ...
 */
export async function processRestaurantCallPayout(
  sharedCallData: any,
  ...
  await admin.firestore().runTransaction(async (tx) => {
    ...
    const officeCurrent = officePointsSnap.data()?.balance || 0;
    const restCurrent = restSnap.data()?.points || 0;
 ...
 */
export async function checkOfficeWalletForClaim(
  provinceId: string,
  cityId: string,
  officeId: string,
