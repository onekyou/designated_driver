...
class MyFirebaseMessagingService : FirebaseMessagingService() {
    ...
    override fun onMessageReceived(remoteMessage: RemoteMessage) {
        ...
        // 일부 알림 유형은 포그라운드 상태에서도 팝업/알림이 필요하다
        val handleEvenInForeground = setOf("NEW_CALL", "NEW_CALL_WAITING")

        if (isAppInForeground() && messageType !in handleEvenInForeground) {
            Log.d(TAG, "앱 포그라운드 & 무시 대상 메시지: $messageType")
            return
        }
        ...
        when (messageType) {
            "NEW_CALL", "NEW_CALL_WAITING" -> handleNewCall(remoteMessage, callId)
