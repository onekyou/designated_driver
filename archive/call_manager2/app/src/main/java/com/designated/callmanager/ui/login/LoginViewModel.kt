...
class LoginViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private suspend fun ensureAdminDoc(uid: String): Pair<String, String>? {
        val prefRegion = sharedPreferences.getString("regionId", null)
        val prefOffice = sharedPreferences.getString("officeId", null)

        if (prefRegion.isNullOrBlank() || prefOffice.isNullOrBlank()) {
            // 설정이 안 돼 있으면 null 반환해서 상위 호출부에서 오류 처리
            Log.e("LoginViewModel", "❌ SharedPrefs 에 region/office 설정이 없습니다. admin 문서 생성 불가")
            return null
        }
        ...
        val needUpdate = snapshot.exists() && (
            snapshot.getString("associatedRegionId") != prefRegion ||
            snapshot.getString("associatedOfficeId") != prefOffice
        )

        if (needCreate || needUpdate) {
            val data = hashMapOf(
                "associatedRegionId" to prefRegion,
                "associatedOfficeId" to prefOffice,
                "updatedAt" to com.google.firebase.Timestamp.now()
            )
            if (needCreate) data["createdAt"] = com.google.firebase.Timestamp.now()
            adminRef.set(data).await()
            Log.i("LoginViewModel", "✅ admin 문서 ${if (needCreate) "created" else "updated"} for $uid -> $prefRegion/$prefOffice")
        }
        return Pair(prefRegion, prefOffice)
    }
