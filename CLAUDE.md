...
3. [WARNING] → 로컬 변경사항 있음 (로컬에서는 수정하지 않으므로 reset --hard로 되돌리기)
...
| 환경 | 경로 | 역할 |
|------|------|------|
| **로컬 (이 환경)** | `C:/app_dev/designated_driver` | pull(덮어쓰기)만 → 빌드 → 기기 설치 → 테스트 |
| **IDX** | 클라우드 | 코드 수정 → commit → push |

### 워크플로우
```
[IDX] 코드 수정 → commit → push
            ↓
[로컬] "시작해줘" → pull(덮어쓰기) → 빌드 → 기기 설치 → 테스트
            ↓
[IDX] 테스트 결과 반영하여 추가 수정
```

### ⚠️ 필수 주의사항
1. **로컬에서는 코드 수정하지 않기** - pull 시 덮어쓰기됨
2. **코드 수정은 IDX에서만** - commit & push도 IDX에서
3. **로컬은 빌드/테스트 전용** - 테스트 결과만 확인하고 IDX에서 반영
