...
## 로컬-IDX 분산 개발 환경
로컬 하드 쓰기 문제로 인해 **분산 개발 워크플로우**를 사용 중입니다.

### 역할 분담
| 환경 | 역할 |
|------|------|
| **IDX (클라우드)** | 코드 수정 → git push |
| **로컬 (여기)** | git pull → 빌드 → adb install → 로그 확인 |

### 워크플로우
1. IDX Claude가 코드 수정 후 `git push`
2. 로컬 Claude(나)가 `git pull` → `./gradlew installDebug` → 기기 설치
3. 로컬 Claude(나)가 `adb logcat`으로 로그 확인 및 보고
4. 필요시 로컬에서도 수정 후 `git push`
