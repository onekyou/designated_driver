...
> **★ 폴더 작업 규율 (2026-06-10 원규씨 명시·반드시 준수)**: **이 폴더(designated_driver)에서 진행하는 활성 작업 = 대리운전(PTT·정산·콜 운영) 전용.** 본류(통화예약·미용·식당)는 **다른 폴더(coupon_app)에서 진행 중** — 이 폴더 세션에서 미용실/본류를 "오늘 할일/다음 작업"으로 끌어오지 말 것(통화예약 엔진 자산은 부품창고로 *보존만*, 본류 진행은 coupon_app). "오늘 할일?" 답할 때 **여기서 제시하는 후보 = 대리운전 항목만**.
...
> **사장 측 행동0 축 = 업종이 도구를 가른다 (2026-06-09 정정).** ① **대리·택시 = PTT 무전기**(매니저 발화→기사 자율입력, 매니저 콜카드 0필드. 위임 대상=기사 있음. 행동0 유일 정합 — `principle_user_action_minimum` 5/25. 이미 양방향 구현 `e41486a0`). ② **미용·식당 = 통화예약 STT**(통화→자동 써머리→[확인] 한 탭→예약 데이터 추출. 위임 대상 없고 예약정보가 목적. 종착 = coupon_app 이식). ⚠️ 6/8 "통화예약 = 미용·대리·택시 공통 입력층"은 *대리를 STT로 잘못 확장한 드리프트*(5/25 PTT 결론을 덮음) → 정정. 검증: 통화예약=coupon_app 미용 / PTT=콜매니저 대리(양평).
