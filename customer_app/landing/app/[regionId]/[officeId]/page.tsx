...
export default function LandingPage() {
  ...
  const loadOfficeInfo = async () => {
    try {
      // Firebase에서 사무실 정보 실제 로드
      const { getDoc, doc } = await import('firebase/firestore')
      const officeDoc = await getDoc(doc(db, 'regions', regionId, 'offices', officeId))

      if (officeDoc.exists()) {
        const data = officeDoc.data()
        setOfficeName(data.name || '대리운전')
      } else {
        setOfficeName('대리운전')
      }
    } catch (error) {
      console.error('사무실 정보 로드 실패:', error)
      setOfficeName('대리운전')
    }
    setIsLoading(false)
  }
  ...
  const handleDownload = () => {
    ...
    window.location.href = `https://play.google.com/store/apps/details?id=com.designated.customer&referrer=${regionId}_${officeId}`
  }
