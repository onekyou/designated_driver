...
class HistorySettlementTagVerificationTest {
    ...
    /**
     * Test 2: displayDeposit (실납입) 이월금 반영 검증
     * carryOver가 있는 경우 adjustedDeposit이 올바르게 계산되는지 검증
     */
    @Test
    fun displayDeposit_withCarryOver_calculatesCorrectly() {
        // Firestore에서 현재 기사의 이월금 조회
        val driverDoc = Tasks.await(
            firestore.collection("provinces").document(provinceId)
                .collection("cities").document(cityId)
                .collection("offices").document(officeId)
                .collection("designated_drivers").document(driverAuthUid)
                .get()
        )

        val carryOverBalance = if (driverDoc.exists()) {
            (driverDoc.getLong("carryOverBalance") ?: 0).toInt()
        } else {
            0
        }

        Log.d(TAG, "이월금: $carryOverBalance")

        // COMPLETED 콜로 rawFinalDeposit 계산
        val snapshot = Tasks.await(
            getCallsCollection()
                .whereEqualTo("status", "COMPLETED")
                .whereEqualTo("driverAuthUid", driverAuthUid)
        ...
        if (snapshot.isEmpty) {
            Log.d(TAG, "COMPLETED 콜 없음 - displayDeposit = 0")
            return
        }
        ...
        snapshot.documents.forEach { doc ->
            ...
            totalFare += fare
            totalCredit += when (paymentMethod) {
                "현금" -> 0
                "현금+포인트" -> {
                    val cash = cashAmount ?: 0
                    if (cash > 0) fare - cash else fare
        ...
        val officeDeposit = SettlementCalc.calculateOfficeDeposit(totalFare, RATIO)
        val rawFinalDeposit = SettlementCalc.calculateRawFinalDeposit(officeDeposit, totalCredit)
        val displayDeposit = SettlementCalc.calculateAdjustedDeposit(rawFinalDeposit, carryOverBalance)

        Log.d(TAG, "rawFinalDeposit=$rawFinalDeposit, carryOver=$carryOverBalance, displayDeposit=$displayDeposit")

        // displayDeposit 검증
        assertTrue("displayDeposit >= 0", displayDeposit >= 0)

        // settlement_today_displayDeposit testTag 값 = "%,d원".format(displayDeposit)
        val displayText = "%,d원".format(displayDeposit)
        Log.d(TAG, "settlement_today_displayDeposit: $displayText")

        // 이월금이 양수(미수령금)인 경우: displayDeposit <= rawFinalDeposit
        if (carryOverBalance > 0 && rawFinalDeposit > 0) {
            assertTrue("미수령금 공제 후 납입 <= 원래 납입",
                displayDeposit <= rawFinalDeposit)
        }

        // 이월금이 음수(미납금)인 경우: displayDeposit >= rawFinalDeposit
        if (carryOverBalance < 0 && rawFinalDeposit > 0) {
            assertTrue("미납금 추가 후 납입 >= 원래 납입",
                displayDeposit >= rawFinalDeposit)
        }
    }
