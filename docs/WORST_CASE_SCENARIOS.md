...
[앱 호출 실패 시]
1. 에러 메시지 표시: "콜 요청 중 오류가 발생했습니다"
2. 손님이 [전화호출] 버튼으로 대체 가능
```

**추가 보완 불필요 이유:**
1. 기사가 직접 손님에게 전화하므로 FCM 알림은 정보성
2. 앱 호출 실패 시 에러 표시 + 전화호출 버튼 존재
3. 전화 = 최종 백업 (항상 작동)
...
| 2 | 손님 상태 변화 미감지 | 단일 문서 리스너 추가 | 매우 낮음 | ⚠️ 미구현 |
| 3 | FCM 토큰 저장 실패 | 재시도 로직 추가 | 없음 | ⚠️ 미구현 |

### 중간 (안정화 후 적용)

| # | 문제 | 해결책 | 비용 |
|---|------|--------|------|
| 4 | Firestore 쓰기 실패 | WorkManager 재시도 큐 | 낮음 |
| 5 | 동시 배차 문제 | 트랜잭션 사용 | 없음 |
| 6 | 연결 상태 미표시 | 연결 상태 UI 추가 | 없음 |

### 낮음 (필요시 적용)

| # | 문제 | 해결책 | 비용 |
|---|------|--------|------|
| 7 | 제조사 강제 종료 | 주기적 폴링 백업 | 낮음 |
| 8 | UI/서버 불일치 | 동기화 상태 배지 | 없음 |
...
2. ⚠️ **손님앱**: 활성 콜 문서 리스너 1개 추가 필요 (FCM 백업)
3. ⚠️ **공통**: FCM 토큰 저장 재시도 로직 필요

손님앱 리스너와 FCM 토큰 재시도만 추가하면 FCM 실패에도 안정적으로 동작합니다.
