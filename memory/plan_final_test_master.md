...
| 단계 | 내용 | 상태 | 비고 |
|------|------|------|------|
| 1단계 | TypeScript 정산 시뮬레이션 | ✅ 완료 | 7일 350콜, 47CP |
| 2단계-기반 | testTag 추가 + androidTest 설정 | ✅ 완료 (`124df476`) | 3/16 |
| 2단계-코드 | JVM Unit Test + Compose UI Test | ✅ 완료 (`5f87dd8c`) | 49개 테스트 |
| 2단계-실행 | 실기기 3대 androidTest | ✅ 완료 (3/18) | |
| 3단계-FCM | FCM data-only + 앱킬→수신 + 재부팅 자동복구 | ✅ 완료 (3/17~3/18) | `e2e0c3d0`, `f5ab8ddc`, `1ce0e1b2` |
| 3단계-Presence | CF presence 전 구간 보호 실기기 검증 | ✅ 완료 (3/18) | `46b2f205` — 4/4 ALL PASS |
| 3단계-전화감지 | 3대 Detector 중복감지 + 부재중 + 연속전화 | ⏳ 파일럿에서 검증 | v9 Phase C |
| 3단계-네트워크 | 오프라인 시나리오 (A1~A4) | ⏳ 파일럿 1주차 이후 | Firestore 오프라인 캐시 기본 동작, LTE 커버리지 관찰 후 대응 |
| (배포전) | Firebase Test Lab 호환성 | 미시작 | |
