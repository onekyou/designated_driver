...
- **★ ② 다음 = sherpa-onnx 한국어 zipformer 평가(Fable이 찾은 놓친 카드)**: k2-fsa **KsponSpeech 전용 학습**(한국어 대화체 970h), **int8 ~126MB(small 190MB보다 작음)**, Transducer라 환각 체질 적음 + **hotwords(지명·기사명 가중) 진짜 지원**. "더 작게+더 충실" 동시 가능 유일 카드. 충실도 미측정 → 기존 `score-batch.mjs` 하니스로 1~2일 스파이크 후 small과 교체 판단. (안 함: base 보정으로 small 대체=불가, 구글/삼성 내장=거인 의존, NPU 가속=이식성.)
