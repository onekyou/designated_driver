...
> ## ⚠️ 5/27 본인 결정 — 자산 재배치 (내부콜 → 공유콜)
>
> 본인 발화 (2026-05-27): *"현재 내부콜을 음성메모와 파싱으로 전달하는데 이는 ptt로 대체하기로 한거고, 공유콜을 현사용중인 음성메모와 파싱으로 전환한다는거지"*
>
> ### 결정 요지
> - **내부콜 STT 입력** → 5/25 PTT plan(`harmonic-sparking-hedgehog.md`)으로 *PTT 발화 대체*. CallMemoParser + 음성메모는 내부콜에서 *역할 종료*.
> - **공유콜 업로드** → 본 plan의 자산(CallMemoParser Phase B-ext + 음성메모 Firebase Storage)을 *공유콜 업로드 흐름으로 이식*.
> - **자산 폐기 0**. *자리만 이동*.
>
> ### 진입점 신규 자리
> - call_manager 공유콜 업로드 화면에 *음성메모 녹음 + STT 파싱* 분기 추가
> - 받는 사무실(공유콜 클레임)에서 *음성 파일 + 파싱된 콜 필드* 동시 수신
>
> ### 채팅 블랙박스(PTT plan §9)와는 별도
> - 채팅 블랙박스 = PTT 발화 → SpeechRecognizer → chat_messages 자동 게시 + 음성 첨부 (블랙박스 패러다임, 오역 OK)
> - 본 plan의 CallMemoParser = STT 결과 *파싱*해서 콜 카드 필드 자동 입력 (정확도 중요)
> - 둘 다 살아남음
>
> ### 실제 코드 진입 시점
> - 5/19 결정 ("PTT 이전 콜마당 수정 전면 보류") 유지
> - 본 결정은 *문서 차원*만, 코드 변경 0
> - 실제 코드 작업은 PTT plan §3 진입 시점과 동시 또는 그 후
>
> ### 관련 자리
> - 본 세션 흐름: `settlement_session_2026-05-27.md`
> - PTT plan: `C:\Users\kala1\.claude\plans\harmonic-sparking-hedgehog.md`
>
> ---
