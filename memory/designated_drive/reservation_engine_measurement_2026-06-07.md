...
## 다음 단계 (★ 2026-06-08 재조정 — 아래가 최신, 위 측정은 6/7 저녁 작성이라 이 재조정 전임)
> 6/8 본인 재조정·확정 3건: ① 콜매니저 파일럿 우선(거친현장은 PC자가샘플 아닌 실파일럿에서만) ② 세 트랙=엔진 하나+세 표면 ③ 비전=최종 앱·웹 완충. C vs W는 **끝 — W 확정**(오늘 증명). C로 후퇴 금지.
1. **★ 다음 할일 = 콜매니저(대리)에 통화예약 엔진 붙이기 = 첫 실전 파일럿** (플랜모드로 설계부터). **W경로 확정**, STT는 **서버 STT**(폰 녹음→서버 전사→Gemini, 앱 가벼움·온폰 STT통합은 나중). 단계: parseReservation deploy(+Vertex IAM aiplatform.user) → 콜매니저 reservation 골격(CallRecordingRepository→ReservationParseClient→ReservationTestScreen, 현 미작동) 실연결 → 써머리확인 UX(통화끝→자동배너→[확인]한탭→콜입력) → 기존 콜 워크플로(NewCallInputDialog 등) 접점 → **실제 양평 대리통화로 현장 검증**. 거친샘플·GO바는 이 파일럿 안에서 자연 확보.
2. **세 트랙 = 엔진 하나 + 세 표면**: 대리(현장 성숙)에서 엔진 실전 → 택시(같은 출발·도착·요금, 거의 공짜) → 미용(화면만 바꿔 최소모드, 도장·손님거점은 나중). 분산 위험 = "엔진을 세 번 만드는 중복"만 막으면 됨.
3. **비전 정합(2026-06-08 확정, CLAUDE.md 최상위 박음)**: 최종 = 손님·상인 둘 다 **앱**. 웹앱 = **완충지대**(NFC→웹 마찰0 진입→앱 정착). 행동0 ≠ 앱 회피. 콜매니저=검증장, 엔진은 로컬마루 **앱** 모듈로 이식(본진=coupon_app). 통화예약 써머리 최종접점=앱, 문자/배너=완충·알림. 🟡 미정=웹앱→앱 전환 트리거(coupon_app/CLAUDE.md에 플레이스홀더).
4. **GO 바 확정** = 파일럿 거친 통화로(제안: 사활적중 ≥90%·거짓양성 ≤5%). production STT 경로(서버 우선, 온폰 나중)도 파일럿 중 확정.
5. (선택·후순위) C경로 천장비교 — W가 이미 거의 만점이라 우선순위 낮음. GCS경유 or GCP내부망(deploy 후).
