...
→ **"P5~P8 재구현" 금지** (중복·회귀). 정산 트랙 잔여 = 위 3건 = **보급 재개 준비 작업**.
⚠️ **처분 정정(2026-06-10, 본 §11.7 작성 직후)**: 초기 "실사용 X → 우선순위 낮음·파킹"은 오류(원규씨 지적) — ① 1사무실도 기사 여럿이라 게이트 유효 ② 미용/식당은 별 모듈(통화예약)·대리 정산 무관 ③ "단순화+PTT+보급 재개=적극 사용 경로"(원규씨 "정리되는대로 다시 보급"). 위 "양평 정산 실사용 X"는 현재 사실일 뿐 보류 근거 아님. 공통 트리거=**보급 재개 임박**. 상세·트리거 = 결정대장 `_decisions.md` "정산 단순화" [잠정·보급 준비].
