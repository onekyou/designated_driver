...
### [확정 보강·2026-06-19] PTT 블랙박스 전사 = faster-whisper 유지 — Gemini-오디오 직접전사 측정에서 환각·비용폭주 확인
- 계기: "서버로 갈 거면 PII 어차피 폰 떠나니 Gemini(발전 중) 어떠냐 + 우린 억지해석보다 발음 그대로가 낫다"(원규씨). → 통념으로 갈아타지 않고 **실데이터 측정**(이전 구글STT 통념회귀 함정 경계).
- 방법: S21 실통화 6건(미지번호 실손님·단골·총알대리 기사 — 양원규 자가녹음 제외=천장편향 탈피) 발음충실 전사 비교. 신규 `functions/scripts/transcribe-gemini.mjs`(Gemini-2.5-flash 오디오 직접, "보정·추측 금지" 프롬프트) vs `transcribe-whisper.py`(large-v3-turbo). 샘플=`C:\Users\kala1\stt_compare_2026-06-19\`.
- 결과: **깨끗·긴 통화(보험사 s3·정비 s4)는 Gemini 약우세**(정확도; s4 whisper가 "오일 갈아"→"5일 가르쳐" 치명오청). **그러나 짧은 실 대리호출(s5 '용문역→양평터미널')에서 Gemini가 "데려다줄게" 수백회 환각 루프 → 뒷부분("픽업이 없어") 유실 + 토큰 6842개=24원(평소 ~1원의 24배).** whisper는 6건 전부 폭주0·우아한 오청(범위 유지)·s5 끝부분까지 포착.
- 결론: **PTT = 짧고 거친 무전 = Gemini 폭주 조건 = whisper 강점 조건.** 분쟁·기록용 블랙박스엔 "우아한 오청"이 "구간유실+비용폭주"보다 안전. ∴ PTT용 STT는 **faster-whisper 유지**가 실데이터로 입증(통념 아님). 측정이 [확정]을 뒤집기는커녕 보강.
- Gemini-오디오는 **통화예약(미용·식당, 길고 깨끗 + 해석이 장점)용 후보로 분리 유지**(C경로 위 [확정] 줄과 정합). PTT 재도전 시 전제 = maxOutputTokens 상한(비용폭주 차단)+반복억제 프롬프트+루프감지 whisper 폴백 후 재측정.
...
### [확정·2026-06-19 구현·실폰검증] PTT 블랙박스 전사 = 온폰 whisper + 모델 자동 다운로드(포그라운드 서비스)
- **맥락**: PTT→텍스트 온폰 whisper가 190MB 모델을 `adb push` 해야만 켜져 → 케이블·개발자모드 없이 원격 보급 불가(유일 병목). 모델을 폰에서 빼는 길로 **서버 STT(Cloud Run) 먼저 시도**(transcribePtt callable + ServerTranscriber 구현·배포·실폰).
- **서버 시도 결과 → 기각(원규씨 결정)**: 서버 전사는 작동했으나(latencyMs 40~75s, 서버 로그 `[stt] 전사 완료`) **Cloud Run 콜드스타트 75초**(large-v3-turbo CPU 로드)가 Firebase callable 기본 70초 타임아웃을 넘겨 끊김. 상시워밍(비용)·작은모델(정확도)·warm도 40초 = 운영 복잡도↑. 원규씨 "서버가 더 복잡해진다, 온폰 자동다운로드가 낫지 않나" → **온폰+자동다운로드 채택**. 근거: ① 온폰 전사 엔진은 이미 작동(새 코드 = 다운로더뿐) ② 콜드스타트·서버비·네트워크 0, 오프라인 가능 ③ PTT는 사무실 폰 2~3대(우리가 세팅)라 "기기별 편차"(6/9 클라우드 채택 근거) 관리됨 ④ 서버도 같은 large-turbo CPU라 안 빠름(40~75s) = 서버의 "더 빠름"은 환상.
- **6/9 "보편 STT = 클라우드" 와의 관계**: 그 결론은 *예약엔진(손님 대량 폰)* 맥락. **PTT(사무실 제공 폰)는 온폰이 맞음** = 용도가 가름, 모순 아님. 클라우드는 예약엔진(coupon_app)에서 유효.
- **구현(`b...` 미커밋)**: 모델 2개(`ggml-small-q5_1.bin` 190MB + `ggml-silero` 0.9MB)를 Firebase Storage `ptt_models/` 호스팅(`storage.rules` 인증 read 배포). 앱 `ModelDownloader`(filesDir에 없으면 getFile→크기검증→.part→rename 원자적·멱등) + **`ModelDownloadService` 포그라운드 서비스(dataSync)** — 로그인 시 트리거(`modelsMissing` 게이트). WhisperTranscriber 전사 경로 무변경(filesDir의 `ggml-*.bin` 자동 탐색).
- **실폰 검증(2026-06-19, S21·S22 양앱)**: 모델 삭제→앱 실행(로그인)→자동 다운로드→정확 크기 복원(190085487+885098). **★백그라운드 완주 검증**: 코루틴 버전은 백그라운드서 3.4MB 정지(OS 네트워크 스로틀) → **포그라운드 서비스로 격상(원규씨 B 선택)** → 백그라운드(HOME)서도 call_manager 완료·S22 3.4MB→81MB 진행 = 스로틀 해결.
- **서버 자산 처분**: `transcribePtt` Cloud Run 함수는 **배포된 채 휴면 보존**(예약엔진 STT 패턴 자산). 앱의 ServerTranscriber/swap은 git checkout 원복(온폰 복귀). `functions/src/handlers/pttStt.ts`·`stt.ts`는 예약엔진용 유지.
- 바꾸려면: 온폰이 사무실 저사양폰에서 너무 느리거나(전사 RTF 과대), 모델 다운로드가 현장 네트워크서 반복 실패하면 → 하이브리드(약한폰 서버폴백) 재검토.
...
### [확정·2026-06-17 원규씨 확인] 사무실 로스터 — 실 파일럿 vs 테스트 구분
- **실 파일럿 = `OyLNNY8GbFExPHHLkuMK` ("총알대리")** — `provinces/gyeonggi/cities/yangpyeong/offices/OyLNNY8GbFExPHHLkuMK`, status=active, 대리2+픽업2명. **보급/시드/검증의 진짜 대상은 여기.**
- `nEkf0X9g3LZtRX94Mrzu` (양평 활성, 양세훈 등) = **원규씨 테스트용**. `RUbeBEvGGYP5wMhJHhMF` (1004) = 테스트 사무실. 둘 다 보급 대상 아님.
- ⚠️ 과거 메모리/결정대장이 "양평 실사무실 = nEkf"로 보급 대상을 잘못 지목 → 정정. 보급 작업 시 officeId 반드시 `OyLNNY8GbFExPHHLkuMK` 확인.
...
### [확정·2026-06-19 구현·실폰E2E검증·배포] 블랙박스 system 메시지 = 푸시(매 이벤트) → 풀(채팅 열 때 1회 증분)
- **맥락(원규씨 지적)**: "블랙박스를 언제까지 늘어놓을 수는 없어 + 매 로그인 전량 read면 read 폭발 + 로컬 보관?". 조사 결과 **read는 이미 효율적**(Local-First: Room 캐시 + `loadInitialMessages`는 Room empty시만 50건 1회 + 신규=FCM push INSERT, 일상 read≈0 + cleanup 7일 보존). **진짜 비용 = `onChatMessageCreated`가 type무관 전부 멤버토큰 3컬렉션 read + FCM 팬아웃** → 시스템 메시지 콜당 ~6건이라 "이벤트×멤버×콜"로 곱해짐(파일럿 무시 가능, 다중 보급서 증가). ∴ 블랙박스를 늘려도 비용 안 늘게 = **푸시 제거**.
- **결정**: `type:"system"`(자동 이벤트, 고빈도·평소 안 봄) = **풀 전용**(채팅 시트 열 때 증분 동기화). `ptt`(라이브 전사)·`""`(사용자 채팅) = **현행 푸시 유지**(라이브 체감). 인라인 렌더 UX 무변경(데이터 출처만 push→pull).
- **구현(feat(chat) 2026-06-19/20 커밋)**: ① 서버 `chat.ts onChatMessageCreated`에 `if(msg.type==="system") return`(팬아웃 skip, write는 그대로). ② 클라 3앱 `ChatRepository.syncSince`(createdAt>watermark ASC limit300 → `mapDoc` → insertAll REPLACE 멱등 → watermark=max) + `lastChatSyncAt` SharedPrefs(**풀에 의해서만 전진** → 라이브 추월에도 누락0, 미설정시 `getMaxCreatedAt(Room)` 초기화=업글 후 7일치 재read 방지) + `ChatViewModel.syncMessages` + **시트 expand 트리거**(`LaunchedEffect(isExpanded)` — VM.init 아님: VM은 Activity 1회 생성이라 init은 1회뿐 = 누락검토서 잡은 치명 버그).
- **★실폰 E2E 검증(2026-06-19, S21+)**: 테스트 system 메시지 write → functions 로그 `system(블랙박스) — 팬아웃 skip`(이전 메시지 `tokens=3,sent=3` 대비 토큰read·FCM 0) + S21 logcat system FCM INSERT 없음 + 시트 열기 `syncSince 증분 INSERT 1건`(채팅 등장) + 재시작 재열람 `신규 0건`(watermark 영속·재read 0). 4단 전부 통과.
- 바꾸려면: 시스템 메시지를 라이브로 즉시 봐야 하는 요구가 생기면(현재 분쟁·놓침 사후조회용이라 풀로 충분). (선택 미적용: cleanup type별 보존 차등=system 더 길게 — 분쟁 대비, 원규씨 판단 시.)
...
- **→ [확정·2026-06-15 구현·실폰검증] 지명 가제티어(사전) 후처리 1차 = 지명만, 자모 거리 + 헷갈리면 보존**:
  ...
  - **★ 보급 인계(누락 방지)**: **보급 시 실 사무실마다 `node functions/scripts/seed-gazetteer.js <officeId>` 실행 필수** — 미시드면 사전 빈 set → 교정 no-op(무해하나 효과 0). rules(settings/gazetteer read)는 전역 배포라 OK.
  - **★ 사무실 정체성 정정(2026-06-17 원규씨 확인)**: **실 파일럿 = `OyLNNY8GbFExPHHLkuMK`("총알대리", status=active, 대리2+픽업2명)** — gyeonggi/yangpyeong 경로. `nEkf0X9g3LZtRX94Mrzu`(양평 활성)·`RUbeBEvGGYP5wMhJHhMF`(1004)는 **원규씨 테스트용**(보급 대상 아님). 이전 줄의 "양평 실사무실(nEkf)을 보급 대상"으로 본 서술은 오류 → 정정. **✅ 총알대리(OyLNNY) 가제티어 시드 완료(2026-06-17, 22개 지명, before=문서없음→신규생성)**.
...
- **✅ 실폰 회귀 완료(2026-07-05, 1004 테스트 사무실)**: 클라이언트 인증 경로(REST, Admin SDK 아님=rules 실평가) 5종 실측 전부 PASS — ①공유콜 게시 200 ②양성:자기 사무실 수임 200 ③음성:타 사무실 claimedOfficeId 조작 수임 403 ④포인트 자가충전 403(if false) ⑤포인트 read 200(무변경). 스크립트=scratchpad verify-rules-client.mjs. 폰(S21+ call_manager) logcat 권한거부 = 전부 무관 확인: 콜드실행시 calls/designated_drivers 거부=Auth복원 레이스(read규칙 백업본과 바이트동일·복원후 소멸), 재실행 2건=tokenRefreshRequests(백업본에도 규칙 없던 default-deny=기존). → 봉인이 정상 read/수임 무손상 확정.
