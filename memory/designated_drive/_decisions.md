...
- 갭: ★PTT 발화↔콜 연결(빈카드 자동생성/음성명령 "○○기사 가") + screen-off 송신 + 콜드 단축 + 정산 단순화 P5~P8 + 블랙박스 9-B(콜상태→채팅 자동기록).
...
- 🚩선결: 어느 폰이 통화 받나 — PTT 송신(매니저폰)이 통화도 받는 폰이어야 자연. 콜디텍터 전용폰이면 트리거 전달 필요.
