...
- ⚠️ **우선순위 가드**: 온폰 프로브 = *병렬 측정*. **최우선은 여전히 양평 실손님 거친 통화 GO바**(field). 온폰이 GO바를 대체·지연시키지 않게(`feedback_rabbithole_scope_2026-06-04`).
...
- ⚠️ 단 6/7~9 샘플은 본인 자가녹음(천장편향) — **진짜 GO바 = 양평 실손님 거친 통화**는 미확보(다음).
