 ...
 *  ★ 엔진은 CallManagerApplication 단일 소유 — onDestroy에서 pttManager.release() 호출 금지(MainActivity와 공유).
 *  (driver_app DriverForegroundService PTT 부분 이식)
 */
class PttReceiverService : Service() {

    private val pttManager get() = CallManagerApplication.getInstance().pttManager
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        when (intent?.action) {
            ACTION_PTT_JOIN_CHANNEL -> {
                ...
                val regionId = prefs.getString("provinceId", null)
    ...
    companion object {
        ...
        const val ACTION_PTT_JOIN_CHANNEL = "com.designated.callmanager.ACTION_PTT_JOIN_CHANNEL"
        const val ACTION_PLAY_VOICE_MEMO = "com.designated.callmanager.ACTION_PLAY_VOICE_MEMO"
