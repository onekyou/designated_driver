...
object PresenceManager {
    ...
    private const val PRESENCE_PATH = "presence/managers"
    ...
    /**
     * 특정 기사의 Presence 상태 조회
     * @param driverId 기사 ID
     * @param callback 상태 콜백 (status, lastSeen)
     */
    fun getDriverPresence(driverId: String, callback: (String?, Long?) -> Unit) {
        val driverPresenceRef = database.getReference("presence/drivers/$driverId")
        driverPresenceRef.get().addOnSuccessListener { snapshot ->
            val status = snapshot.child("status").getValue(String::class.java)
            val lastSeen = snapshot.child("lastSeen").getValue(Long::class.java)
            callback(status, lastSeen)
        }.addOnFailureListener { e ->
            Log.e(TAG, "기사 Presence 조회 실패: $driverId", e)
            callback(null, null)
        }
    }
}
