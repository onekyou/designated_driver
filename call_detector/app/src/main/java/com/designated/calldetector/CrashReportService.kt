...
class CrashReportService(private val context: Context) {
    ...
    private val provinceId: String by lazy {
        val detectorConfig = context.getSharedPreferences("detector_config", Context.MODE_PRIVATE)
        val callDetectorPrefs = context.getSharedPreferences("CallDetectorPrefs", Context.MODE_PRIVATE)

        detectorConfig.getString("provinceId", null)
            ?: callDetectorPrefs.getString("provinceId", null)
            ?: "province_test" // 테스트용 기본값
    }

    private val cityId: String by lazy {
        val detectorConfig = context.getSharedPreferences("detector_config", Context.MODE_PRIVATE)
        val callDetectorPrefs = context.getSharedPreferences("CallDetectorPrefs", Context.MODE_PRIVATE)

        detectorConfig.getString("cityId", null)
            ?: callDetectorPrefs.getString("cityId", null)
            ?: "city_test" // 테스트용 기본값
    }
    ...
    fun initialize() {
        ...
        crashlytics.log("Province ID: $provinceId")
        crashlytics.log("City ID: $cityId")
        crashlytics.log("Office ID: $officeId")

        // 디바이스 정보 설정
        crashlytics.setUserId(deviceId)
        crashlytics.setCustomKey("province", provinceId)
        crashlytics.setCustomKey("city", cityId)
    ...
    private fun reportCrashToFirestore(exception: Throwable) {
        try {
            ...
            // Crashlytics Custom Keys에 모든 크래시 정보 저장
            crashlytics.setCustomKey("crash_device_id", deviceId)
            crashlytics.setCustomKey("crash_province_id", provinceId)
            crashlytics.setCustomKey("crash_city_id", cityId)
            ...
            crashlytics.log("Location: Province=$provinceId, City=$cityId, Office=$officeId")
    ...
    private fun sendEmergencyAlertSync(crashTime: Long): com.google.android.gms.tasks.Task<Void> {
        val emergencyAlert = hashMapOf(
            ...
            "timestamp" to FieldValue.serverTimestamp(),
            "crashTime" to crashTime,
            "message" to "콜디텍터 [$deviceId] 강제종료 발생!",
            "requiresImmediateAction" to true,
            "provinceId" to provinceId,
            "cityId" to cityId,
            "officeId" to officeId
